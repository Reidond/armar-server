...
|---|---|---|
| Realtime model ID | `src/core/ai/config/` | Match the model ID string against the current docs |
| Session config params | `src/core/realtime/session/` | Turn detection fields, audio format fields |
| Realtime event types | `src/core/realtime/session/openai_event_router.py` | Event names match current schema |
| Chat completions model | `src/core/ai/chat/` | Model ID, response format, tool use schema |
| Audio format | `src/core/realtime/audio/` | Sample rate, encoding (pcm16 vs others) |
