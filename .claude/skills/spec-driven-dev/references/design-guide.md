...
`vertical-slice` skill (`.claude/skills/vertical-slice/SKILL.md`) for the
canonical feature directory template.

Key constraints:
- Feature code lives in `src/features/{feature_name}/`
...
- [ ] Route registered in the feature's `routes.py`, included via `src/api/api.py`
