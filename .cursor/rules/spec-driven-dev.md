...
If the task involves new external services or AI providers, research them before writing specs:
fetch official API docs, verify SDK method signatures, document capabilities/constraints.
For multi-provider tasks, force an explicit strategy decision (single active, fallback chain, etc.).
Skip if the task uses only providers already integrated.
